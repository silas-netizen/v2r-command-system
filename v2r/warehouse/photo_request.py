...
SCENES: tuple[str, ...] = (
    # 사용자 규칙(2026-09-19): 연출된 상차림 금지, 화면의 대부분은 음식·제품 자체.
    "제품(음식) 자체를 화면 가득 담은 클로즈업, 접시나 컵 하나만, 배경은 거의 안 보이게",
    "먹기 직전 위에서 내려다본 구도, 음식이 화면의 80% 이상, 소품 없음",
    "한 입 먹고 남은 모습을 가까이서, 숟가락이나 포크 하나만 걸침",
    "포장을 막 뜯어 내용물이 보이는 모습을 가까이서, 손 일부만 살짝",
    "컵이나 그릇에 담긴 모습을 45도 각도로 가까이, 테이블은 흐릿하게",
    "조리 중이거나 막 완성된 모습을 가까이서, 김이나 물기 같은 질감 위주",
)
...
STYLE_RULES = (
    ...
    "사람 얼굴이 나오지 않게, 음식·제품 클로즈업 위주, 4:3 또는 3:4 비율, 선명한 고해상도"
)
...
ANTI_AI_RULES = (
    ...
    "평범한 사람이 스마트폰으로 찍은 스냅샷처럼 보여야 하되, 초점은 정확하고 선명해야 한다. "
    "음식·제품이 주인공이다. 상차림 연출, 배경 소품 배치, 사람·손·인테리어가 화면을 차지하는 구도 금지. "
    "자연광 또는 실내 조명 그대로, 화이트밸런스가 살짝 어긋나도 좋다. "
    "중앙 정렬·완벽한 대칭 금지, 피사체를 살짝 치우친 구도로. "
    "글자·문구·로고·라벨·워터마크·서명 넣지 말 것. "
    "3D 렌더·CG·일러스트·과한 보케·유리알 같은 반짝임·HDR 과보정·매끈한 플라스틱 질감 금지. "
    "인위적인 스튜디오 조명, 완벽한 그림자, 잡티 없는 표면 금지. 모션 블러 금지."
)
