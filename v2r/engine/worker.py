...
def _naver_keepalive(rt: Runtime, spec: TaskSpec) -> dict:
    """네이버 로그인 세션을 점검·연장하고, 풀렸으면 알린다 (사용자 규칙 2026-09-21)."""
    from v2r.warehouse.naver_session import RELOGIN_NOTICE, check_naver_session
...
def dispatch(rt: Runtime, job: Any, owner: str | None = None) -> dict:
    ...
    if task == "sync_article_index":
        from v2r.engine.article_sync import sync_all_cafes
        ...
        if "전체" in notes:
            ...
            # 기존 문구("글 목록 동기화", "자사 카페 글 목록 동기화")는 자사만 — 하위 호환
            scope = "self"
        out = sync_all_cafes(rt, scope, with_bodies="본문까지" in notes)
