...
```
기본은 검증 모드(dry run). `실제 발행` 또는 `바로 등록`처럼
`(실제|바로)(발행|등록)` 형태로 써야만 실제로 등록한다.
`실제로 해줘`, `바로 올려` 같은 표현은 **모의 실행으로 남는다**(안전 불변식).
JSON 명령으로 `dry_run: false`를 보내도 `notes`에 위 문구가 없으면 모의 실행이다.
