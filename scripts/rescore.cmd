@echo off
chcp 65001 > nul
rem 브랜드 6개(우아덤·코숨핏·뉴더미스·장으뜸·팥순이·갱년기) 연관도 채점을 클로드 채점
rem 워커(브랜드당 1개)와 Codex 교차검증 워커(브랜드당 2개)로 분리해 동시에 띄운다
rem (사용자 지시 2026-09-25: 클로드 채점이 GPT 검증 속도에 발목 잡히지 않도록).
rem 갱년기는 2026-09-26 추가(제품 미정, 원고 발행 없음 — 키워드 채점만 같은 방식으로).
rem 총 6*(1+2)=18개 프로세스. 중복 실행 방지는 파이썬 쪽 잠금
...
rem 갱년기(2026-09-26 추가, 원고 발행 없음 — 키워드 채점만)도 같은 방식으로 채점한다
for %%B in (우아덤 코숨핏 뉴더미스 장으뜸 팥순이 갱년기) do (
