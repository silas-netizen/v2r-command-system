...
- **태그**: 제휴·브랜드 글의 태그는 **키워드 1개뿐**(공백 모두 제거, `tags_from_keyword`). 키워드는 A열 `키워드` → 비면 G열 `말머리`(말머리를 안 쓰는 카페의 브랜드 시트는 키워드를 G열에 적는다 → 그 행의 `head`는 빈 값) 순. 둘 다 비면 태그 없이 발행하고 경고만 남긴다(결과에 `태그 없음`). 일상 글은 행에 키워드가 있을 때만 태그를 단다. 등록 검증은 `tag_list`를 그대로 비교한다.
- **제휴 체인 되감기**: 일상 글 등록 뒤 수정글이 확정되기 전에 끊기면(NoPhotoError·seone `ValueError`·카탈로그 오류·모호하지 않은 API 오류) 방금 만든 일상 글을 `delete_article`로 지우고(최선 노력, 실패는 로그만) `failed` + 단계 `등록 전 실패(일상 글 되감기): …`, 결과·이벤트에 `rolled_back_daily=<id>`. 수정글 POST가 모호한 오류(`ambiguous`/네트워크)면 지우지 않고 `uncertain`으로 둔다(reconcile 담당). 되감기를 줄이려고 수정글 재료(사진 수 vs 자리표시, `content_json`, 댓글, 카탈로그 해석)는 일상 글 등록 **전에** 미리 점검한다.
