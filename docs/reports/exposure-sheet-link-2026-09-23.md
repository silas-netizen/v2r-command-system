...
- 영향받은 3개 파일(`test_keyword_exposure_cycle.py`+`test_engine.py`+`test_parser.py`)만 묶어
  ...
  일으킨 실패는 0건.
- 전체 스위트(1,343개)도 백그라운드로 끝까지 돌린 결과(41분 14초, 다른 일꾼 프로세스와 자원을
  다투느라 평소보다 오래 걸림) **최종: 1,343 passed, 0 failed, 11 errors**. 11건 전부 같은
  장부 파수꾼(`tests/conftest.py:52`, `data/llm_usage-2026-09.jsonl` 크기 변화 감지) 테어다운
  오탐 — `test_brand_writer.py`·`test_bulk_generate.py`·`test_plan_command.py`·`test_engine.py`
  등 이 작업과 무관한 파일들에서 발생했고, 전부 다른 일꾼의 `keyword_relevance` 전량
  재산정 프로세스가 그 시간에도 실시간으로 `llm_usage` 로그에 쓰고 있었기 때문(로그에
  `키워드 연관도 채점 실패 브랜드=우아덤` 등 그쪽 작업 흔적이 같이 찍혀 있음). **FAILED는
  0건** — 이 연결 작업이 일으킨 실패는 없다.
