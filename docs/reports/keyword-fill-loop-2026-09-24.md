...
- 우아덤·코숨핏·뉴더미스 새 워커가 사라진 원인 확인(백그라운드로 재현 중).
- 옛 워커 3개가 회차를 마치면 새 워커가 잠금을 넘겨받아 새 조건으로 돈다. 즉시 끝내려면 사용자 결정 후 프로세스 종료가 필요하다.
- Codex 적체 해소(교차검증 병렬화 또는 묶음 축소)가 없으면 원고 대상 수는 늘지 않는다.
