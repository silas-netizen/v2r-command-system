...
## 4-2-1. 승인 흐름 (생성 전·후 두 번 물어본다)

사용자 규칙(2026-09-19): **키워드 폴더에 있는 사진을 먼저 세탁해서 쓰고, 폴더가
비었거나 모자랄 때만 만든다. 만들기 전에 먼저 묻고, 만든 사진도 승인을 받는다.**

1. **부족 알림** — 발행(실제 발행만, 모의 실행은 제외) 중 `publish.pick_images`가
   미사용 세탁본을 못 찾으면 `publish.notify_photo_shortage`가
   `사진 부족: 브랜드 <브랜드> / <폴더> 폴더 — 필요 N장, 사용 가능 M장. 생성하려면
   '사진 생성 승인 <브랜드> <폴더> N장' 이라고 보내세요` 를 보낸다.
   같은 브랜드/폴더는 **한 실행에 한 번만** 알린다(`rt.scratch["photo_shortage_notified"]`).
   자동 생성은 하지 않고 그 슬롯은 예전처럼 실패한다(제휴 체인에서는 일상 글보다
   앞선 계획 단계라 되감을 글이 없다).
2. **`사진 N장 생성 …`(승인 없음)** — `worker._generate_photos`가 `generate_batch`를
   **부르지 않고** `stock.inventory`로 그 폴더의 원본·세탁본·미사용 수만 알려 준다.
   모자라면 승인 문장을, 충분하면 "생성하지 않았습니다"를 덧붙인다.
3. **`사진 생성 승인 <브랜드> <키워드> N장`** — `TaskSpec.approved=True`.
   `generate_batch(..., collect=False)`로 만들어 파일을 `warehouse/inbox/new/<브랜드>/<키워드>/`에
   그대로 둔다(**세탁도 적재도 하지 않는다**). 이어서 사진을 한 장씩
   `channels.notify_photo_all`(텔레그램 `sendPhoto`)로 보내며 캡션에
   `<브랜드>/<키워드> i/n — 승인: '사진 승인 …' / 반려: '사진 반려 …'` 를 붙인다.
   사진 전송이 안 되는 채널뿐이면 경로를 글로 보낸다.
4. **`사진 승인 <브랜드> <키워드>`** — `photo_request.collect_new(brand=…, keyword=…)`로
   **그 폴더만** 적재 + 세탁하고 건수를 답한다.
5. **`사진 반려 <브랜드> <키워드>`** — `photo_request.reject_new`가 그 인박스 폴더의
   이미지 파일만 지운다. `inbox/new/` 밖은 절대 지우지 않고, 창고의 원본·세탁본도
   건드리지 않는다.
